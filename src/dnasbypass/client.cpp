...
void DNASBypass::Client::requestConnect(const std::vector<unsigned char>& request, const atomizes::HTTPMessage& http_request, const UrlRequest::UrlVariables& url_variables)
{
	...
	std::vector<unsigned char> request_file(request.end() - content_length, request.end());
	
	// Sign data
	std::vector<unsigned char> signature;
	Sign(request_file, signature);
	
	atomizes::HTTPMessage http_response;
	
	http_response.SetStatusCode(200);
	//http_response.SetHeader("Server", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
	//http_response.SetHeader("Host", "gate1.us.dnas.playstation.org");
	//http_response.SetHeader("Accept-Ranges", "bytes");
	http_response.SetHeader("Content-Type", "image/gif");
	
	std::string response = http_response.ToString();
	response = response.substr(0, response.length() - 2);
	response += "Content-Length: " + std::to_string(signature.size()) + "\r\n\r\n";
	
	std::vector<unsigned char> result;
	result.reserve(response.size() + signature.size());
	std::copy(response.begin(), response.end(), std::back_inserter(result));
	std::copy(signature.begin(), signature.end(), std::back_inserter(result));
	this->Send(result);
	...
	for(int i = 0; i < request_file.size(); i++)
	{
		ss << std::hex << std::setfill('0') << std::setw(2) << (int)(request_file[i]);
	}
	Logger::debug("request_file = " + ss.str());
	
	ss.str("");
	for(int i = 0; i < signature.size(); i++)
	{
		ss << std::hex << std::setfill('0') << std::setw(2) << (int)(signature[i]);
	}
	Logger::debug("signature = " + ss.str());
...
std::vector<unsigned char> example_A_file_name =
{
...
void DNASBypass::Client::Test()
{
	...
	std::vector<unsigned char> packet1 = example_A_file_name;
	std::vector<unsigned char> packet2;
	std::vector<unsigned char> packet3;
	
	GetFileName(request, game_id, query_type, file_name);
	GetChecksums(request, chksum1, chksum2);
	GetKeysAndSeed(chksum1, chksum2, des_key1, des_key2, des_key3, xor_seed);
	
	packet2 = encrypt3(packet1, 0xc8, 0x20, des_key1, des_key2, des_key3, xor_seed);
	packet3 = encrypt3(packet2, 0x28, 0x120, des_key4, des_key5, des_key6, xor_seed2);
...
/*
	Generate checksums
*/
void DNASBypass::Client::GetChecksums(const std::vector<unsigned char>& request, std::vector<unsigned char>& chksum1,
	std::vector<unsigned char>& chksum2)
{
	SHA1(&(request[0x34]), 0x100, &(chksum1[0]));
	SHA1(&(request[0x48]), 0xec, &(chksum2[0]));
}

void DNASBypass::Client::GetFileName(const std::vector<unsigned char>& request, std::string& game_id, std::string& query_type,
		std::string& file_name)
{
	...
	for(int i = 0; i < 8; i++)
	{
		ss  << std::hex << std::setfill('0') << std::setw(2) << (int)(request[i + 0x2c]);
		
		if(i < 4)
		{
			ss2 << std::hex << std::setfill('0') << std::setw(2) << (int)(request[i]);
		}
	}
	game_id = ss.str();
	query_type = ss2.str();
...
void DNASBypass::Client::Sign(const std::vector<unsigned char>& request, std::vector<unsigned char>& packet3)
{
	std::string   game_id;
	std::string   query_type;
	std::string   file_name;
